...
exports.connect = function(baseURI, username, password, callback) {
  ...
  adapter.log.debug("Connecting to Gardena Smart System Service at " + baseURI + " ...");
...
exports.http_put = function(uri, json2send, callback) {
  ...
  request(options, function (err, response, jsondata) {
    if (err) {
      ...
      adapter.log.debug('Command has been sent successfully! Answer: ' + JSON.stringify(jsondata));
