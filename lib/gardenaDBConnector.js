  ...
  adapter.getDevices(function(err, devices) {
    ...
    for(let i=0;i<devices.length;i++) {
      ...
      if(in_commands.length > 0) {
        ...
        let cat_commands = jsonPath.query(gardena_commands, '$[?(@.category=="' + device_cat[0].category + '")]');

        for(let j=0;j<cat_commands.length;j++) {
          createCommandStatesInDB(cat_commands[j], devices[i], device_cat[0].category);
        }
      }
    }
  });
...
function createCommandStatesInDB(cat_command, device, category, callback) {
  let baseURI = cat_command.baseURI;
  ...
  // setup the commands
  let commands_to_create = cat_command.commands;
