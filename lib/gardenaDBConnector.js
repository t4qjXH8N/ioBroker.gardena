...
const conf_seperator = '_'; // separator to evaluate datapoints from the config
...
function createSmartDatapointsStateInDB(curid, state, callback) {
  let id = curid.split('.')[curid.split('.').length-1];
  let stateid = curid.replace('.' + id, '') + '.' + state.name;
...
exports.syncDBDatapoints = function (datapoints, callback) {
      ...
      adapter.getStates('gardena.' + adapter.instance + '.datapoints.*', function (err, states) {
        ...
        for(let cstate in config_states) {
          ...
          if(config_states[cstate].hasOwnProperty('smart') && config_states[cstate].smart && smart_datapoints) {
            ...
            createSmartDatapointsStateInDB(dp_prefix + cstate.replace(conf_re_seperator, '.'), config_states[cstate]);
          } else {
